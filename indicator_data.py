...
def make_indicator_options(kind, code, range_key="1Y"):
    range_map = {
        "1M": ("3mo", 1),
        "3M": ("6mo", 1),
        "1Y": ("1y", 1),
    ...
    if kind == "market":
        period, _ = range_map.get(range_key, ("1y", 1))
        frame = market_history(code, period)
        title = MARKET_LABELS.get(code, code)
    else:
        _, years = range_map.get(range_key, ("1y", 1))
        frame = macro_history(code, years)
        title = MACRO_LABELS.get(code, code)

    if frame.empty:
        raise RuntimeError("표시할 지표 데이터가 없습니다.")
    ...
    options = {
        ...
        "grid": {"left": 58, "right": 22, "top": 28, "bottom": 55},
        "xAxis": {
            "type": "category",
            "data": labels,
            "boundaryGap": False,
        ...
        "series": [{
            ...
            "showSymbol": False,
            "smooth": False,
            "lineStyle": {"width": 2, "color": "#5da9e9"},
            "areaStyle": {"color": "rgba(93,169,233,.10)"},
        }],
    }
