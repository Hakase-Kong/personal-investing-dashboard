...
def _client() -> Client:
    if not SUPABASE_URL or not SUPABASE_PUBLISHABLE_KEY:
        raise RuntimeError("SUPABASE_URL / SUPABASE_PUBLISHABLE_KEY가 필요합니다.")
...
def load_watchlist(session_store):
    client = _authenticated_client(session_store)
    response = (
...
def add_watchlist(session_store, item):
    client = _authenticated_client(session_store)
...
def delete_watchlist(session_store, market, exchange, symbol):
    client = _authenticated_client(session_store)
    return (
...
def sign_out(session_store):
    try:
        _authenticated_client(session_store).auth.sign_out()
    finally:
        for key in [
